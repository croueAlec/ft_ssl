...
static void	set_byte_color(size_t separator_index, size_t current_index)
{
	...
	} else if (current_index >= sha256_PADDED_CHUNK_SIZE && (separator_index < sha256_PADDED_CHUNK_SIZE || separator_index == sha256_CHUNK_SIZE + 2))
	{
		p_print_trace("\033[0;94m");
	}
	else if (current_index < separator_index && separator_index != sha256_CHUNK_SIZE + 2)
	{
