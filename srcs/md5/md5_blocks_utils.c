...
static uint32_t	uint8_to_uint32_little_endian(const uint8_t *buf)
{
...
static void	set_byte_color(size_t separator_index, size_t current_index)
{
	...
	} else if (current_index >= md5_PADDED_CHUNK_SIZE && separator_index < md5_PADDED_CHUNK_SIZE)
	{
...
void	print_md5_block_chunk(const t_block_md5 *block, size_t separator_index)
{
	...
	for (size_t i = 0; i < 64; i++)
	{
		...
		if ((i+1) % 8 == 0) {
			p_print_trace("\033[0;1m\t   0x%08x  0x%08x\033[0m\n", uint8_to_uint32_little_endian(&block->chunk[i-7]), uint8_to_uint32_little_endian(&block->chunk[i-3]));
		}
		else if (i != 63)
			p_print_trace(" ");
	}
